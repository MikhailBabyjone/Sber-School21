...
void max_rows(int** matrix, int n, int m, int* max_row);
void min_cols(int** matrix, int n, int m, int* min_col);
void min_max_output(int* a, int size);
...
int main() {
    ...
    min_col = (int*)malloc(m * sizeof(int));
    if (max_row == NULL || min_col == NULL) {
        ok = 0;
    }
    if (ok && input(matrix, n, m) == 0) {
        ok = 0;
    }
    if (ok) {
        max_rows(matrix, n, m, max_row);
        min_cols(matrix, n, m, min_col);
        output(matrix, n, m);
        printf("\n");
        min_max_output(max_row, n);
        printf("\n");
        min_max_output(min_col, m);
    }
